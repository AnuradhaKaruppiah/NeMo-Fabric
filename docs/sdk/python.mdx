...
| `Service` | A Fabric-owned or caller-owned adapter service shared by one or more runtimes. | Prepare or attach it, pass it to `start_runtime(...)`, then release it after all connected runtimes stop. |
...
Adapters that advertise service support can also prepare a Fabric-owned service
...
| `Fabric.prepare_service(config, base_dir=...)` | Yes | Fabric should configure, start, and supervise a shared adapter service. | Returns a Fabric-owned `Service`. Release stops the service after every connected runtime stops. |
| `Fabric.attach_service(config, reference, base_dir=...)` | Yes | A deployment system already owns the service. | Returns a caller-owned `Service`. Release detaches Fabric without stopping the external service. |
...
an attachment only removes Fabric-owned clients and temporary state.
