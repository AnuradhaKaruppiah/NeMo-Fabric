...
- The listener correlates records to one NeMo Relay scope tree. Deep Agents and
  Hermes roots carry the NeMo Fabric request ID. Remote Agent services receive
  that ID in `x-nemo-fabric-request-id` and must attach it as
  `nemo_fabric_request_id` metadata to their root Agent scope. For Claude and
  Codex, the SDK matches NeMo Relay turn scopes by their role and 1-based turn
...
installation and receive the Fabric-managed ATOF sink URL with each streaming
request; the local adapter does not install or start Relay for the service.
