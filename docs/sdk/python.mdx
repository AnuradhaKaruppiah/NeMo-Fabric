...
`doctor(...)`, `run(...)`, `start_runtime(...)`, `prepare_environment(...)`, or
`attach_environment(...)`. The base
...
| `Fabric.start_runtime(config, ..., streaming=False)` | Yes | You need state across multiple ordered invocations. | Returns a `Runtime`. Use `streaming=True` with NeMo Relay enabled only to provision Relay ATOF streaming. |
| `Fabric.prepare_environment(config, *, base_dir=...)` | Yes | A development flow lets Fabric create an explicitly configured environment. | Returns an `EnvironmentHandle`; the caller decides when to release it. |
| `Fabric.attach_environment(config, reference, *, base_dir=...)` | Yes | A deployment already owns an environment and wants Fabric to verify and bind it. | Accepts a typed `EnvironmentReference`; Fabric does not gain deletion authority. |
| `Fabric.start_runtime_in(config, environment, ...)` | Yes | You need one runtime bound to an explicitly prepared or attached environment. | Runtime shutdown does not release the environment. |
| `Fabric.release_environment(environment)` | Yes | You are finished with an explicit environment handle. | Deletes only Fabric-owned resources; caller-owned resources are detached. |
...
## Explicit Non-Local Environments

Environment ownership is independent of a Fabric runtime session. For a
self-contained development flow, `prepare_environment(...)` can create the
configured environment. For a deployment flow, the consumer can provision the
environment itself and pass a typed, provider-specific reference to
`attach_environment(...)`:
...
try:
    async with await fabric.start_runtime_in(
        ...
        result = await runtime.invoke(input="Inspect the repository")
finally:
    await fabric.release_environment(environment)
```

Attachment verifies the referenced resource through its environment provider;
it does not create the resource or transfer deletion authority. Runtime stop
does not release either a prepared or attached environment. For a caller-owned
handle, `release_environment(...)` detaches Fabric and leaves final deletion to
the consumer. See the
[LangGraph OpenShell vertical slice](https://github.com/NVIDIA/NeMo-Fabric/tree/main/examples/langgraph_openshell_poc)
for a runnable example of this deployment-oriented flow.
