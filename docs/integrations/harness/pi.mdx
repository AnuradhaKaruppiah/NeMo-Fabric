...
harness version selected by that project:
...
The adapter package includes the process entry point and descriptor. Point NeMo
...
For source dependency setup, install only the Pi adapter workspace and its
pinned development harness:

```bash
just install-typescript-pi
```

The full TypeScript build installs all maintained TypeScript workspaces before
building them:
...
Use `adapters/typescript/pi/pi.fabric-adapter.json` for source discovery.
...
## Add a NeMo Fabric Tool Definition
