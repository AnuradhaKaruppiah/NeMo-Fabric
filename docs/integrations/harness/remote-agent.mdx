...
The collector URL must be `http://<fabric-host>:<port>/atof` and reachable from
the remote deployment. Fabric binds the collector listener when the runtime
starts; it does not start or configure the remote deployment's Relay sink. The
remote Relay configuration must publish to the same URL. The adapter adds
