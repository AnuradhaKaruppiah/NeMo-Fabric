...
| NVIDIA-labs Object Oriented Agents (NOOA) CodingAgent | `--variant nooa` | Installed NOOA and `nooa-cli`, exposed `external/nooa/src` on `PYTHONPATH`, and set `NVIDIA_API_KEY` |
...
Relay Python package in their selected adapter environment. The NOOA
...
The NOOA variant selects the registered `nvidia.nooa.coding-agent` target.
