...
test("recovers an opaque proxy error only when context is near the model limit", () => {
  assert.match(
    classifyOpaqueProxyContextOverflow("500 status code (no body)", 181_000, 262_144, 65_536),
    /maximum context length is 262144 tokens/u,
  );
  assert.equal(
    classifyOpaqueProxyContextOverflow("500 status code (no body)", 150_000, 262_144, 65_536),
    "500 status code (no body)",
  );
  assert.equal(
    classifyOpaqueProxyContextOverflow("503 status code (no body)", 200_000, 262_144, 65_536),
